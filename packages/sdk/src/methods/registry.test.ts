...
describe('Test Registry', () => {
  ...
  it('should set metadata to domain', async () => {
    ...
    const nickname = await contract.getMetadataByKey(
      domain,
    ...
    expect(nickname).toBe(metadata[MetadataKeys.CONTACT_NICKNAME]);
  ...
  it('should change owner whithout nft correctly', async () => {
    ...
    const { balanceNftHandle, assetId, transactionRequest } =
      await contract.simulateTxNftHndler({
        domain,
    ...
    const transactionResponse = await owner.sendTransaction(transactionRequest);
