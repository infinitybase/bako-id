import { Address, TransactionStatus, ZeroBytes32, bn } from 'fuels';
...
describe('[METHODS] Registry Contract', () => {
  ...
  it('should error on call method without a proxy contract started', async () => {
    ...
    const registry = await TestRegistryContract.deploy(owner);

    expect.assertions(2);

    try {
      const callFn = await registry.functions
        .register(domain, owner.address.toB256(), 1)
