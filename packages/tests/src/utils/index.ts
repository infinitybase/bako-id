import { BaseAssetId, bn, BN, Provider, Wallet, WalletUnlocked } from 'fuels';
import { RegistryContractAbi__factory, StorageContractAbi__factory, TestContractAbi__factory } from '../types';
...
import { registryContract, storageContract, testContract } from '../types/contract-ids.json';
...
async function setupContractsAndDeploy(wallet: WalletUnlocked) {
  ...
  const register = async (domain: string, account: string, calculateAmount = true) => {
    const amount = domainPrices(domain);
    const callBuilder = registry
      .functions
  ...
  const connect = (wallet: WalletUnlocked) => StorageContractAbi__factory.connect(storage.id, wallet);
