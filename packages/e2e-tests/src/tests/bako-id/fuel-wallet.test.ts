...
test.describe('Connect with Fuel Wallet', () => {
  ...
  // Bug changing ownership
  test.fixme(
    'edit resolver and ownership',
    async ({ page, context, extensionId }) => {
      ...
      await test.step('edit resolver', async () => {
        ...
        await page
          .locator('div')
          .filter({ hasText: /^AddressesEdit$/ })
          .getByRole('button')
          .click();
        await page.getByRole('textbox', { name: 'Address' }).fill(address2);
        await page.waitForTimeout(500);
        await page.getByRole('button', { name: 'Save' }).click();
        await expect(page.getByText('actionEdit Resolver')).toBeVisible();
        await page.getByRole('button', { name: 'Confirm' }).click();
        await E2ETestUtils.signMessageFuelWallet({
          fuelWalletTestHelper,
          page,
        ...
        await page
          .locator('div')
          .filter({ hasText: /^AddressesEdit0x/ })
          .getByRole('img')
          .nth(1)
          .click();
        ...
        await test.step('search new resolver in Fuel Wallet', async () => {
          ...
          await extensionPage
            .getByRole('button', { name: 'Send Button' })
            .click();
          await extensionPage
            .getByRole('textbox', { name: 'Address Input' })
            .fill(`@${newHandle}`);
      ...
      await test.step('edit ownership', async () => {
        await page.reload();
        const connectButton = page.getByRole('button', {
          name: 'Connect Wallet',
        });
